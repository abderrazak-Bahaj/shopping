<template>
  <div class="product-grid">
    <div
      class="product-image wow fadeInUp"
      data-wow-delay="0.4s"
    ...
    >
      ...
      <span class="product-sale-label" v-if="ProductIsNew(product.created_at)"
        >new</span
      >
        <button
        v-if="!hasFavorite(product.id)"
        @click="addFavorite(product.id)"
        class="product-like-icon cls-btn-def"
      ...
      <ul class="product-links">
                      
        <li>
          <router-link :to="'/shope/' + product.id"
    ...
    <div class="product-content">
      <h3
        class="h5 text-center wow fadeInDown"
        data-wow-delay="0.2s"
      ...
      <div
        class="price text-center wow fadeInDown"
        data-wow-delay="0.4s"
        data-wow-duration="1s"
      >
        {{ product.p_price | filterPrice }}
      </div>
    </div>
            
  </div>
  ...
  methods: {
    ...
    deleteFavorite: async function (id_product) {
      ...
      this.$helpers.showToast(
        "Product has removed in favorite success",
        "warning",
      );
    },
    addToCart: async function (id) {
      ...
      if (number.isConfirmed) {
        ...
        if (res == undefined) {
          ...
          await this.$store
            .dispatch("UpdateCartForUser", data)
            .then(() => {})
            .catch(() => {});
        }
        this.$helpers.showToast(
