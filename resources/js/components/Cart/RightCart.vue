<template>
  <div class="container">
    <h5 class="h3">
      <span>Quantity :</span> You have {{ getAllCarts.length }} items in your
      cart
    </h5>
    ...
    <div v-if="!!getAllCards.length">
      ...
      >
        <option v-for="card in getAllCards" :key="card.id" :value="card.id">
      ...
      <div class="row">
        <router-link to="/" class="col-md btn btn-outline-secondary m-2"
          >Return to page hom</router-link
        >
        <button
          @click="bay"
          class="col-md btn btn-outline-success m-2"
          :disabled="calculDeferent"
        >
          bay
        </button>
      </div>
    ...
    <div v-else class="text-center">
      <router-link to="/card" class="btn btn-outline-secondary"
...
export default {
  ...
  computed: {
    ...
    calculDeferent: function () {
      return (
        parseFloat(this.cardSelected.card_sold) < this.getPriceCart ||
        this.cardSelected == ""
      );
    },
  },
