            ...
            <div class="stars col-md" v-if="userHasAuth">
              <form class="row">
                ...
                <label
                  class="star star-5 col"
                  for="star-5"
                  @click="rateProduct(product.id, 5)"
                ></label>
                <input
                ...
                <label
                  class="star star-4 col"
                  for="star-4"
                  @click="rateProduct(product.id, 4)"
                ></label>
                <input
                  class="star star-3"
                  id="star-3"
                ...
                />
                <label
                  class="star star-3 col"
                  for="star-3"
                  @click="rateProduct(product.id, 3)"
                ></label>
                <input
                  class="star star-2"
                  id="star-2"
                ...
                />
                <label
                  class="star star-2 col"
                  for="star-2"
                  @click="rateProduct(product.id, 2)"
                ></label>
                <input
                  class="star star-1"
                  id="star-1"
                ...
                />
                <label
                  class="star star-1 col"
                  for="star-1"
                  @click="rateProduct(product.id, 1)"
                ></label>
              </form>
            </div>
          </span>
...
export default {
  ...
  methods: {
    rateProduct: async function (product_id, rate_Point) {
      await this.$store
        .dispatch("addRate", { product_id, rate_Point })
        .then(() => {
    ...
    by: async function (product_id) {
      ...
      var options = {};
      this.getAllCards.map(
        (o) => (options[o.id] = o.card_number.slice(0, 5) + "********"),
      );
    ...
    addCart: async function () {
      await Swal.fire({
        ...
        if (result.isConfirmed) {
          let res = this.getAllCarts.find(
            (c) => c.product_id == this.product.id,
          );
          if (res == undefined) {
            let formData = new FormData();
