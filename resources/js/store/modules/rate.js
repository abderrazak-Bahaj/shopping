const rate = {
  ...
  getters: {
    findRate: (state) => (product_id) =>
      state.rates.find((r) => r.product_id == product_id),
  },
  mutations: {
    SET_RATES(state, rates) {
      state.rates = rates.data.map((r) => {
        let total =
          Number.parseInt(r.total_point) / Number.parseFloat(r.count_point);
