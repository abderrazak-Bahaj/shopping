...
const product = {
  ...
  getters: {
    ...
    getLinksProducts: (state) => (type) => {
      let list = state.PaginationLinks.map((item) => {
        return {
          url: type + item.url?.slice(item.url?.indexOf("?")) || "/",
          label: item.label,
          active: item.active,
        };
      });
  ...
  mutations: {
    SET_PRODUCTS(state, products) {
      ...
      state.PaginationLinks = products.meta.links;
      state.IsLoaded = true;
    },
    ...
    UPDATE_PRODUCT(state, product) {
      state.products[state.products.indexOf((r) => r.id == id)] = product.data;
    },
    ...
    DESTROY_PRODUCT(state, id) {
      state.products.splice(
        state.products.findIndex((p) => p.id == id),
        1,
      );
      state.IsLoaded = true;
    },
  ...
  actions: {
    ...
    async createProduct(context, product) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .post(`/api/product`, product, { headers })
          .then((res) => {
          ...
          .catch((err) => {
            ...
            if (err.response.status == "422") reject(err.response.data.errors);
            else context.dispatch("showError", err.response.statusText);
          });
      });
    },
    ...
    async updateProduct(context, product) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .post(`/api/product/update/${product.id}`, product, { headers })
          .then((res) => {
    ...
    async CountProduct(context) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .get(`/api/product/admin/statistic`, { headers })
          .then((res) => {
            resolve(res.data);
          })
          .catch((err) => {
