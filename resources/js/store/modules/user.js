const user = {
  ...
  getters: {
    ...
    EmailIsVerified: (state) =>
      state.user.email_verified_at == null ? false : true,
  ...
  actions: {
          ...
          .then((response) => {
            if (response.status == 200) {
              if (response.data.user.email_verified_at != null) {
                ...
                if (this.getters["userIsAdmin"]) resolve(true);
                else resolve(false);
              } else {
      ...
      return new Promise((resolve, reject) => {
        axios
          ...
          .catch((errors) => {
            ...
            else context.dispatch("showError", errors.response.statusText);
            reject();
          });
      });
    },
    ...
    updateProfile(context, user) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post("/api/user/update", user, { headers })
          .then((response) => {
            if (response.status == 200) {
          ...
          .catch((error) => {
            if (error.response.status == "422")
              ...
              reject(false);
            } else context.dispatch("showError", error.response.statusText);
          });
      });
    },
    /**
    ...
    editPassword(context, data) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .put("/api/user/editpassword", data, { headers })
          .then((res) => {
          ...
          .catch((error) => {
            if (error.response.status == "422")
              reject(error.response.data.errors);
            else if (error.response.status == "401") reject(false);
            else context.dispatch("showError", error.response.statusText);
          });
      });
    },
    ...
    forgotPassword(context, email) {
      return new Promise((resolve, reject) => {
        axios.post("/api/user/forgot-password", email).then(
          (response) => {
            resolve();
          },
          (error) => {
    ...
    emailVerify(context, data) {
      return new Promise((resolve, reject) => {
        axios.get(`/api/email/verify/${data.id}/${data.token}`).then(
          (response) => {
    ...
    emailResend(context, token) {
      ...
      return new Promise((resolve, reject) => {
        axios.post("/api/email/resend", null, { headers }).then(
          (response) => {
            if (response.status == 200) {
              resolve(true);
      ...
      return new Promise((resolve, reject) => {
        axios
          .put(
            ...
            { password: user.password, isAdmin: 1 },
            { headers },
          )
          .then((response) => {
            resolve(response);
    ...
    async CountUser(context) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .get(`/api/user/admin/statistic`, { headers })
          .then((res) => {
            resolve(res.data);
          })
          .catch((err) => {
