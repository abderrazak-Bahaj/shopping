...
const order = {
  ...
  mutations: {
    SET_ORDERS(state, orders) {
      ...
      (state.orderIsLoaded = true), (state.links = orders.meta.links);
    },
    DESTROY_ORDER(state, id) {
      state.orders.splice(
        state.orders.findIndex((o) => o.id == id),
        1,
      );
      state.orderIsLoaded = true;
    },
    ADD_ORDER(state, order) {
      state.orders.unshift(order.data);
    },
    DELETE_ALL_ORDERS(state) {
      state.orders = [];
    },
  },
  actions: {
    ...
    async CountOrder(context) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .get(`/api/order/admin/statistic`, { headers })
          .then((res) => {
            resolve(res.data);
          })
          .catch((err) => {
    ...
    async GreyOrderGetList(context) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .get(`/api/order/admin/list`, { headers })
          .then((res) => {
            resolve(res.data);
          })
          .catch((err) => {
    ...
    async GreyOrderAction(context, action) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .post(
            `/api/orders/admin/${action.id}/update`,
            { State: action.State },
            { headers },
          )
          .then((res) => {
