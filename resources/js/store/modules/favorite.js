const favorite = {
  ...
  getters: {
    findFavorite: (state) => (product_id) =>
      state.favorites.find((f) => f.product_id == product_id),
    hasFavorite: (state) => (product_id) =>
      !!state.favorites.find((f) => f.product_id == product_id),
  ...
  mutations: {
    ...
    DELETE_ALL_FAVORITES(state) {
      state.favorites = [];
    },
    DESTROY_FAVORITE(state, id) {
      ...
      if (userToken) {
        await axios
          ...
          .catch((err) => {
            context.dispatch(
              "showError",
              err.response.statusText + ",error load favorite",
            );
          });
      }
    },
