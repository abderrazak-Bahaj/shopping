...
const category = {
  ...
  mutations: {
    SET_CATEGORAIS(state, categorais) {
      state.categorais = categorais.data;
      state.categoryIsLoad = true;
    },
    ADD_CATEGORY(state, category) {
      state.categorais.push(category.data);
      state.categoryIsLoad = true;
    },
    UPDATE_CATEGORY(state, category) {
      state.categorais[
        state.categorais.indexOf((c) => c.id == category.data.id)
      ] = category.data;
      state.categoryIsLoad = true;
    },
    DESTROY_CATEGORY(state, id) {
      state.categorais.splice(
        state.categorais.findIndex((e) => e.id == id),
        1,
      );
      state.categoryIsLoad = true;
    },
  ...
  actions: {
    ...
    updateCategory(context, data) {
      ...
      return new Promise((resolve, reject) => {
        axios
          .post(`/api/category/${data.id}`, formData, { headers })
          .then((res) => {
