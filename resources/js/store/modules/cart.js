const cart = {
  ...
  getters: {
    ...
    getPriceCart: (state) =>
      state.carts.reduce(
        (a, b) => a + (parseFloat(b["p_price"]) * parseInt(b["quantity"]) || 0),
        0,
      ),
  },
  mutations: {
    ...
    ADD_CART(state, cart) {
      state.carts.push(cart.data);
    },
    UPDATE_CART(state, cart) {
      let object = state.carts.find((c) => c.id == cart.data.id);
      object.quantity = cart.data.quantity;
    },
    DESTROY_CART(state, id) {
      state.carts.splice(
        state.carts.findIndex((e) => e.id == id),
        1,
      );
    },
    DELETE_ALL_CARTS(state) {
      state.carts = [];
    },
  },
  actions: {
      ...
      if (this.getters["userToken"]) {
        await axios
          ...
          .catch((err) => {
            context.dispatch(
              "showError",
              err.response.statusText + ",error load cart",
            );
          });
      } else context.dispatch("showError", "Login first");
    },
    ...
    async addCartForUser(context, data) {
      ...
      await axios
        ...
        .catch((err) => {
          context.dispatch(
            "showError",
            err.response.statusText + ",error add cart",
          );
        });
    },
    // Update Cart
    ...
    async UpdateCartForUser(context, data) {
      ...
      return await new Promise((resolve, reject) => {
        axios
          .put(`/api/cart/${data.id}`, { quantity: data.quantity }, { headers })
          .then((res) => {
