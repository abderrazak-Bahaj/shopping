      ...
      <table class="table table-hover" id="printMe">
        ...
        <tbody>
          <tr v-for="order in allOrders" :key="order.id">
            ...
            <td>
              <button
                class="btn-lg btn-outline-success rounded"
                @click="AcceptOrder(order.id)"
              >
                <i class="fa-solid fa-circle-check"></i>
              </button>
              <button
                class="btn-lg btn-outline-danger rounded"
...
export default {
  ...
  methods: {
    AcceptOrder: async function (id) {
      ...
      await this.$store
        .dispatch("GreyOrderAction", action)
        .then((response) => {
          this.allOrders.splice(
            this.allOrders.findIndex((item) => item.id == response.data.id),
            1,
          );
