      ...
      <table class="table table-hover">
        ...
        <tbody>
          <tr v-for="user in users" :key="user.id">
            ...
            <th>{{ user.email }}</th>
            <td class="row">
              <p v-show="user.isAdmin == 1" class="h6 text-uppercase">
                Is Admin
              </p>
              <button
                class="col btn btn-outline-success m-1"
                :disabled="IsBusy"
                @click="TransformUserToAdmin(user.id)"
                v-show="user.isAdmin == 0"
              >
                <i class="fa-solid fa-rotate"></i> <small>Set Admin</small>
              </button>
              <button
                ...
                v-show="user.isAdmin == 0"
              >
                <i class="fa-solid fa-ban"></i> <small>Delete</small>
              </button>
            </td>
          </tr>
        </tbody>
  ...
  methods: {
    TransformUserToAdmin: async function (id) {
      let res = await Swal.fire({
        ...
        .then((result) => result.value.password)
        .catch(() => null);
      if (res == null) return;
      this.IsBusy = true;
      let action = { id, password: res, isAdmin: 1 };
      await this.$store
        .dispatch("SetAdmin", action)
        .then((response) => {
    ...
    DeleteUser: async function (id) {
      ...
      await this.$store
        ...
        .then((response) => {
          this.users.splice(
            this.users.findIndex((item) => item.id == response),
            1,
          );
          this.$helpers.showToast("Operation has been completed success");
        })
