<template>
  <div class="overflow-auto">
    ...
    <div class="alert alert-danger" role="alert" v-if="hasErrorBack">
      <small
        v-for="(subErrors, index) in errorBackEnd"
        :key="index"
        class="text-danger"
      >
    ...
    <div class="row">
      <div class="form-floating m-1 col">
        <input
          type="text"
          v-model="getProduct.p_name"
          class="form-control cls-txt-input"
          id="p_name"
      ...
      <div class="m-1 col">
        <select
          ...
          v-model="getProduct.category_id"
        >
          <option selected value="0">Open this and select Category</option>
          <option
            v-for="category in getAllCategorais"
            :value="category.id"
            :key="category.id"
          >
            {{ category.cat_name }}
    ...
    <div class="form-floating m-1 col">
      <textarea
        cols="30"
        rows="10"
        v-model="getProduct.p_description"
        class="form-control cls-txt-input"
...
export default {
  ...
  methods: {
    validator: function () {
      this.error = "";
      if (
        this.getProduct.p_name == "" ||
        this.getProduct.p_color == "" ||
        this.getProduct.p_price == "0" ||
        this.getProduct.category_id == 0 ||
        this.getProduct.p_description == "" ||
        this.getProduct.p_image_4 == "" ||
        this.getProduct.p_image_3 == "" ||
        this.getProduct.p_image_2 == "" ||
        this.getProduct.p_image_1 == ""
      ) {
    ...
    editProduct: async function () {
      ...
      if (await this.validator()) {
        await this.$store
          .dispatch("updateProduct", this.getProduct)
          .then((res) => {
            console.log("sss");
