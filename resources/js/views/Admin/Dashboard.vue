<template>
  <div>
    <cart-item-dashboard
      ...
      :countUser="countUser[0].Count + countUser[1].Count"
      :countOrder="
        countOrder[0].Count + countOrder[1].Count + countOrder[2].Count
      "
    />
  ...
  methods: {
    getCountProduct: async function () {
      await this.$store
        ...
        .then((res) => (this.countProduct = res));
    },
    getCountOrder: async function () {
      await this.$store
        .dispatch("CountOrder")
        .then((res) => (this.countOrder = res));
    },
    getCountUser: async function () {
      await this.$store
        .dispatch("CountUser")
        .then((res) => (this.countUser = res));
    },
  },
  created() {
